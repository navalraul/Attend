...
export const Register = async (req, res) => {
    try{
        const { name, email, password,  number } = req.body.userData;

        if(!name || !email || !password || !number) return res.json({ success: false, message: "All fields are mandatory..."})

        const isEmailExist = await UserModels.find({email})
        ...
        const user = new UserModels ({ name, email, password: hashedPassword, number});
